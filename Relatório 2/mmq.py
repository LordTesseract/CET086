...
def main():
    ...
    for i, line in enumerate(entrada):
        line = re.sub('[\n]*','',line)
        line = line.split(";")
        xVal = np.array(line[0].replace('[', '').replace(']','').replace('\n', '').split(',')).astype(np.float)
        ...
        for i in range(systemResultsLength):
            if i < systemResultsLength - 2:
                aproximacao = aproximacao + "{}{}*x**{} ".format(mainOperator, abs(systemResults[i]),systemResultsLength - 1 - i)
                mainOperator = " + " if systemResults[i+1] >= 0 else " - "
            elif i < systemResultsLength - 1:
                aproximacao = aproximacao + "{}{}*x".format(mainOperator, abs(systemResults[i]))
                mainOperator = " + " if systemResults[i+1] >= 0 else " - "
            elif i == systemResultsLength - 1:
                aproximacao = aproximacao + "{}{}".format(mainOperator, abs(systemResults[i]))

        saida = open("result.txt", "a")
        saida.write("---------------Problem Nยบ {}-------------------\n".format(i+1))
        saida.write("Approximate equation found: Y = {}".format(aproximacao))
        saida.write("\n\n")
        saida.close()

        approximateFunction = lambdify(Symbol('x'), parse_expr(aproximacao))

        for i in range(len(xVal)):
            estimativa.append(approximateFunction(xVal[i]))

        plt.scatter(xVal, yValues)
        plt.plot(xVal, estimativa, color='blue')
        plt.show()
